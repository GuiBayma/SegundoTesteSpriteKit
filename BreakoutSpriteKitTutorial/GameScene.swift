...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMoveToView(view: SKView) {
        ...
        for i in 0..<numberOfBlocks {
            let block = SKSpriteNode(imageNamed: "block.png")
            block.position = CGPointMake(xOffset + CGFloat(CGFloat(i) + 0.5)*blockWidth + CGFloat(i-1)*padding, CGRectGetHeight(frame) * 0.73)
            block.physicsBody = SKPhysicsBody(rectangleOfSize: block.frame.size)
            block.physicsBody!.dynamic = false
            block.physicsBody!.allowsRotation = false
            block.physicsBody!.friction = 0.0
        ...
        for i in 0..<numberOfBlocks {
            let block = SKSpriteNode(imageNamed: "block.png")
            block.position = CGPointMake(xOffset + CGFloat(CGFloat(i) + 0.5)*blockWidth + CGFloat(i-1)*padding, CGRectGetHeight(frame) * 0.66)
            block.physicsBody = SKPhysicsBody(rectangleOfSize: block.frame.size)
            block.physicsBody!.dynamic = false
            block.physicsBody!.allowsRotation = false
            block.physicsBody!.friction = 0.0
